...
應用程式精靈已經為您建立了這個 OpenGL 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 OpenGL 應用程式的檔案。
...
OpenGL.h
    ...
    並宣告 COpenGLApp 應用程式類別。

OpenGL.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 COpenGLApp。
...
OpenGLDoc.h、OpenGLDoc.cpp - 文件
    這些檔案包含您的 COpenGLDoc 類別。編輯這些檔案，即可新增您的特
    殊文件資料，並實作檔案儲存和載入 (透過 COpenGLDoc::Serialize)。

OpenGLView.h、OpenGLView.cpp - 文件的檢視
    這些檔案包含您的 COpenGLView 類別。
    COpenGLView 物件是用來檢視 COpenGLDoc 物件。
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 OpenGL.pch 的先行編譯標頭 (PCH) 檔，
